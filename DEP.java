...
public class DEP extends Application implements EventHandler<ActionEvent> {
	...
	private TextField name, manager, budget, depBudget, startDate, title, sponsor, dNumber;
	...
	private void setDepartmentPane() {
		...
		departList.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Integer>() {
			public void changed(ObservableValue<? extends Integer> v, Integer oldV, Integer newV) {
				...
				if (depart != null) {
					...
					depBudget.setText(String.format("%.2f", depart.getBudget()));
		...
		depBudget = new TextField();
		depBudget.setEditable(false);
		departmentPane.addRow(4, depBudget);
	...
	public void start(Stage primaryStage) {
		...
		DepartmentGUI departmentGUI = new DepartmentGUI.Builder()
				.setContainerConfiguration("-fx-background-color: white;", 10, 5, Pos.TOP_LEFT)
				.addContainerRowLabel(0, "Department")
				.addContainerRowLabel(0, "information")
				.addDepartmentNumbers(departments)
				.setDepartmentNumbersView(1, "Department Number", 100, 100)
				.setNameView(2, "Name", false)
				.setManagerView(3, "Manager", false)
				.setBudgetView(4, "Budget", false)
				.setStartDateView(5, "Start Date", false)
				.build();

		departmentGUI.addDepartmentNumbersListener(new ChangeListener<Integer>() {
			public void changed(ObservableValue<? extends Integer> v, Integer oldV, Integer newV) {

				Department depart = getDepartment(newV);

				if (depart != null) {
					departmentGUI.name.setText(depart.getName());
					departmentGUI.manager.setText(String.valueOf(depart.getManager()));
					departmentGUI.budget.setText(String.format("%.2f", depart.getBudget()));
					departmentGUI.startDate.setText(depart.getStartDate());
				}
		...
		root.setLeft(departmentGUI.container);
...
// ArrayList<ArrayList<String>> deconstructedFile =
// this.readFile("departments.txt");
...
//
// Department department = new Department(number, name, manager, budget,
// startDate);
...
// ArrayList<ArrayList<String>> deconstructedFile =
// this.readFile("employees.txt");
// deconstructedFile.forEach((fileline) -> {
// // A, 600, Willy, 01/01/1988, 41 Station Street Wollongong NSW 2500, M,
// 250.5,
// // 0, 9, Cook Read
...
// Admin admin = new Admin(number, name, dob, address, gender, salary,
// supervisor, dNumber,
// skillsOrLanguages);
...
// Developer developer = new Developer(number, name, dob, address, gender,
...
// ArrayList<String> wordsOfLine = new
